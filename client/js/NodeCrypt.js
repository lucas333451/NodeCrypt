...
class NodeCrypt {
	...
	constructor(config = {}, callbacks = {}) {
		...
		this.encryptArchiveMessage = this.encryptArchiveMessage.bind(this);
		this.decryptArchiveMessage = this.decryptArchiveMessage.bind(this)
	}
	...
	// Set auth token for account binding
	setAuthToken(token) {
				...
				if (await crypto.subtle.verify({
					...
					if (this.authToken) {
						try {
							...
							this.logEvent('auth-token', err, 'error')
						}
					}
					this.sendMessage(this.encryptServerMessage({
						a: 'j',
	...
	sendChannelMessage(type, data) {
		if (this.serverShared) {
			try {
				let payloads = {};
				const archivePayload = this.encryptArchiveMessage({
					a: 'm',
					t: type,
					d: data,
					ts: Date.now(),
	...
	// Encrypt a message for history/archive using room password derived key
	encryptArchiveMessage(message) {
		if (!this.credentials || !this.credentials.password) return '';
		let encrypted = '';
		try {
			const key = Buffer.from(this.credentials.password, 'hex'); // sha256 hex => 32 bytes
			let payload = Buffer.from(JSON.stringify(message), 'utf8');
			if ((payload.length % 16) !== 0) {
				payload = Buffer.from([...payload, ...Buffer.alloc(16 - (payload.length % 16))])
			}
			const iv = Buffer.from(crypto.getRandomValues(new Uint8Array(12)));
			const counter = Buffer.from(crypto.getRandomValues(new Uint8Array(4)));
			const cipher = new chacha(key, iv, counter.reduce((a, b) => a * b));
			encrypted = iv.toString('base64') + '|' + counter.toString('base64') + '|' + Buffer.from(cipher.encrypt(payload)).toString('base64')
		} catch (error) {
			this.logEvent('encryptArchiveMessage', error, 'error')
		}
		return encrypted;
	}

	decryptArchiveMessage(message) {
		if (!this.credentials || !this.credentials.password) return {};
		let decrypted = {};
		try {
			const parts = message.split('|');
			const key = Buffer.from(this.credentials.password, 'hex');
			const decipher = new chacha(key, Buffer.from(parts[0], 'base64'), Buffer.from(parts[1], 'base64').reduce((a, b) => a * b));
			decrypted = JSON.parse(Buffer.from(decipher.decrypt(Buffer.from(parts[2], 'base64'))).toString('utf8').replace(/\0+$/, ''))
		} catch (error) {
			this.logEvent('decryptArchiveMessage', error, 'error')
		}
		return decrypted;
	}
