...
import { fetchHistory, getSession } from './util.auth.js';
let roomsData = [];
...
// Join a room
// 加入一个房间
export function joinRoom(userName, roomName, password, modal = null, onResult, authToken = null, userId = null) {
	const newRd = getNewRoomData();
	newRd.roomName = roomName;
	newRd.myUserName = userName;
	newRd.password = password;
	newRd.authToken = authToken;
	newRd.userId = userId;
	...
	if (authToken) {
		chatInst.setAuthToken(authToken);
	} else {
...
// Load historical messages from server (archive ciphertext, decrypt client-side)
async function loadHistoryForRoom(idx) {
	const rd = roomsData[idx];
	if (!rd || !rd.chat) return;
	const dialogId = rd.chat.credentials ? rd.chat.credentials.channel : null;
	if (!dialogId) return;
	const session = getSession();
	const token = (session && session.token) || rd.authToken;
	if (!token) return;
	try {
		const res = await fetchHistory({ dialogId, limit: 100, token });
		if (!res.ok || !res.data || !res.data.items) return;
		for (const item of res.data.items) {
			const decrypted = rd.chat.decryptArchiveMessage(item.cipher);
			if (!decrypted || decrypted.a !== 'm') continue;
			let msgType = decrypted.t || 'text';
			const senderName = decrypted.from || item.sender_id || t('ui.anonymous', 'Anonymous');
			const payload = decrypted.d;
			const timestamp = decrypted.ts || item.ts || Date.now();
			roomsData[idx].messages.push({
				type: 'other',
				text: payload,
				userName: senderName,
			...
			if (activeRoomIndex === idx && window.addOtherMsg) {
				window.addOtherMsg(payload, senderName, senderName, false, msgType);
			}
		}
	} catch (error) {
