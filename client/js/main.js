...
import { sendEmailCode, registerAccount, loginAccount, getSession, setSession } from './util.auth.js';
...
function setupSendCodeButtons() {
	...
	document.body.addEventListener('click', async (e) => {
		...
		if (!target || !target.classList.contains('send-code-btn')) return;
		const prefix = target.getAttribute('data-prefix') || '';
		const emailInput = document.getElementById(`email${prefix}`);
		const email = emailInput ? emailInput.value.trim() : '';
		if (!email) {
			addSystemMsg(t('system.enter_email', 'Please enter email first.'));
			return;
		}
		if (cooldowns[email]) return;
		target.disabled = true;
		const originalText = target.innerText;
		target.innerText = t('ui.sending', 'Sending...');
		const res = await sendEmailCode(email);
		if (res.ok) {
			addSystemMsg(t('system.code_sent', 'Verification code sent.'));
			cooldowns[email] = true;
			let remaining = 60;
			const timer = setInterval(() => {
				remaining -= 1;
				target.innerText = `${remaining}s`;
				if (remaining <= 0) {
					clearInterval(timer);
			...
			addSystemMsg(t('system.code_send_failed', 'Send code failed, please retry.'));
			target.disabled = false;
			target.innerText = originalText;
		}
	});
...
async function handleAuthSubmit({ userName, roomName, password, email, code, authMode, modal, btn }) {
	const mode = (authMode || 'login').toLowerCase();
	...
	try {
		let needsAuth = true;
		if (session && session.token && session.username === userName) {
			needsAuth = false;
		}

		if (needsAuth && identifier && password) {
			let res;
			if (mode === 'register') {
				if (!email || !code) {
					addSystemMsg(t('system.code_required', 'Email and verification code are required for registration.'));
					return resetBtn();
				}
				res = await registerAccount({ username: userName, email, password, code });
			} else {
				res = await loginAccount({ identifier, password });
			}
			if (!res.ok) {
				const err = res.data?.error || 'auth_failed';
				addSystemMsg(`${t('system.auth_failed', 'Authentication failed')}: ${err}`);
				return resetBtn();
			}
			session = getSession();
		} else if (!identifier || !password) {
			addSystemMsg(t('system.auth_required', 'Account login is required to enter.'));
			return resetBtn();
		}

		if (!session || !session.token) {
			addSystemMsg(t('system.auth_failed', 'Authentication failed'));
			return resetBtn();
		}
			...
			if (images.length > 0) {
				...
				if (rd.privateChatTargetId) {
					...
					if (targetClient && targetClient.shared) {
						...
						const archivePayload = rd.chat.encryptArchiveMessage({
							a: 'm',
							t: 'image_private',
							d: messageContent,
							ts: Date.now(),
