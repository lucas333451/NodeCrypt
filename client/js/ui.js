...
export function loginFormHandler(modal) {
	return function(e) {
		e.preventDefault();
		let userName, roomName, password, btn, roomInput, warnTip, email, code, authMode;
		if (modal) {
			...
			email = document.getElementById('email-modal').value.trim();
			code = document.getElementById('emailCode-modal').value.trim();
			authMode = document.getElementById('authMode-modal').value;
			...
			code = document.getElementById('emailCode').value.trim();
