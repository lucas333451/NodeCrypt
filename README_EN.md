...
## 🆕 Account + History (Keeps E2EE)
- Account is required (no anonymous join). Uses email verification code and persists history. Server only stores ciphertext; room password-derived keys decrypt history on the client.
- Without the correct room password, history cannot be decrypted; keep the room password safe.
- Emails are sent via MailChannels (built into Cloudflare Workers). Set `MAIL_FROM` to an address on your domain with proper SPF/DKIM.
- Storage uses D1. Only ciphertext + minimal metadata are stored; no room passwords or plaintext ever hit the server.
...
- `GET /api/history?dialog=<id>&after=<id>&limit=50` // Bearer token required
...
## 🛠️ Cloudflare Workers Deployment (with Auth/History)
1. Install deps and build frontend  
   ```bash
   npm install
   npm run build
   ```
2. Create a D1 database and note its `database_id`:  
   ```bash
   wrangler d1 create nodecrypt-db
   ```
3. Update `wrangler.toml` `d1_databases` entry with your `database_id`.
4. Apply schema (from `worker/db-schema.sql`):  
   ```bash
   wrangler d1 execute nodecrypt-db --file=worker/db-schema.sql
   ```
5. Set environment variable for email sender (MailChannels):  
   ```bash
   wrangler secret put MAIL_FROM   # e.g. no-reply@yourdomain.com (domain must have SPF/DKIM)
   ```
6. Login and deploy  
   ```bash
   wrangler login
   ...
   ```
7. Verify logs and history API:  
   ```bash
   wrangler tail
   # open the site, register/login, send messages, reload and confirm history loads
   ```
